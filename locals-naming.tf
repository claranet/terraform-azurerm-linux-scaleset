locals {
  ...
  vmss_name     = coalesce(var.custom_vmss_name, data.azurecaf_name.vmss_linux.result)
  nic_name      = coalesce(var.custom_nic_name, data.azurecaf_name.nic.result)
  ipconfig_name = coalesce(var.custom_ipconfig_name, data.azurecaf_name.ipconfig.result)
